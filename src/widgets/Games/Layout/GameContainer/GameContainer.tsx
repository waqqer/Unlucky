...
const GameContainer = forwardRef<GameContainerRef, UIGameContainerProps>((props, ref) => {
    ...
    const [bet, setBet] = useState<string>(String(gameBet.default || gameBet.min))
    ...
    const choosePresetHandle = useCallback((value: number) => {
        if (!StateMachine.is("IDLE")) return
        setBet(String(value))
    }, [StateMachine])
    ...
    const buttonDisabled: boolean = useMemo((): boolean => {
        ...
        if (Number(bet) < gameBet.min || Number(bet) > gameBet.max)
            return true
                ...
                {type === "double" ?
                    <div className={styles["d-box"]}>
                        ...
                        <div className={styles.input}>
                            <h2>Ставка</h2>
                            <Separator size={50} />
                            <Input type="number" value={bet} min={gameBet.min} max={isAuth ? Math.min(gameBet.max, balance) : gameBet.max} onChange={onBetInputChange} className={styles["bet-input"]}/>
