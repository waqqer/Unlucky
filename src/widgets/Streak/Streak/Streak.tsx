...
interface UIStreakProps {
    custom?: { streak: number, status: StreakStatus },
    tooltip?: boolean
}

const Streak = (props: UIStreakProps) => {
    const {
        custom,
        tooltip = true
    } = props

    const { streak, streakStatus } = useContext(AccountContext)
    const uniqueId = useId()

    if (streakStatus === "DEAD") {
        return
    }
    ...
    return (
        <>
            <div
                className={`${styles.streak} ${streakStatus === "WAITING" && styles.wait}`}
                id={`streak-${uniqueId}`}
            >
            ...
            {tooltip && streakStatus !== "WAITING" &&
                <Tooltip
                    anchorSelect={`#streak-${uniqueId}`}
                    portalRoot={document.getElementById("tooltips")}
                    delayShow={100}
                    className={styles["streak-tooltip"]}
                    opacity={1}
                >
                    ...
                    <div className={styles["streak-title"]}>
                        <h4>Дней</h4>
                    </div>
                </Tooltip>
            }
