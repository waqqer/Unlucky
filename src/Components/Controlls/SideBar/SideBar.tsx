...
interface SideBarProps extends Parent {
    onOpen?: () => void
    onClose?: () => void
}
...
const SideBar = (props: SideBarProps) => {
    ...
    return (
        <>
            <div
                className={`${styles.side} ${isOpen ? styles.opened : ""}`}
                onMouseEnter={openHandle}
                onMouseLeave={closeHandle}
            >
                <svg className={styles.logo} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
            ...
            <div
                className={`${styles["side-overlay"]} ${isOpen ? styles.opened : ""}`}
            >
            </div>
        </>
    )
